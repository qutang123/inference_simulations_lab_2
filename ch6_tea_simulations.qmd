...
based_on_conditions = tea_data |> group_by(condition)|>
  summarize(mean_r = mean(rating),
            sd_r = sd(rating),
            n =n())
...
based_on_conditions_highn = tea_data_highn |>
  group_by(condition) |>
  summarize(mean_r = mean(rating),
            sd_r = sd(rating),
            n =n())
based_on_conditions_highn
```
...
```{r}
ggplot(tea_data_summary, aes(x=delta)) +
...
```{r}
ggplot(tea_data_highn_summary, aes(x=delta)) +
  geom_histogram(alpha=.5, bins=10) 
```
...
ggplot(null_summary, aes(x=delta)) +
  geom_histogram(alpha=.5, bins=10) 
```
...
# Answer: It represents each simulation.
```
...
# Answer: As same as the null distribution (mean = 0).
```
...
Answer: Each dot reprsents a rating score in one experiment.

To do: how does this change when you use the low-n experiment?

Answer: The range of the confidence interval will increase, and the value of se will also increase.
...
ggplot(data = tea_data_highn, aes(x=condition, y=rating, col=condition))  +
  ...
  theme_minimal() +
  ggtitle('Tea ratings across conditions with SE')
...
ggplot(data = tea_data_highn, aes(x=condition, y=rating, col=condition))  +
  ...
  ggtitle('Tea ratings across condition with CIs')
...
1.  Now go back (earlier in the code) and modify the DELTA in the simulation functions to be smaller so that there is only a small difference between groups. Is it still significant?
    ...
    Answer: No, it is not significant.

2.  Rewrite this code with the the smaller sample size simulations. What changes?

    Answer: The distribution is less like the normal distribution.
